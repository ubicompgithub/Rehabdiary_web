...
<script language="javascript" type="text/javascript">
  ...
  <div id="information">
      <h2 style="text-align: center;">病患反思狀況總覽</h2>
 ...
 <div class="main">
  ...
  <div id="footer">
  	<div class="categorybtn-group" style="top: 20px; text-align: center" id="record_btn_group">
            <button id="neg_btn"  class="btn" onclick="changeCategory(this);">負面情緒</button>
            <button id="phy_btn"  class="btn" onclick="changeCategory(this);">生理因素</button>
            <button id="pos_btn" class="btn" onclick="changeCategory(this);">正面情緒</button>
            <button id="ctrl_btn"  class="btn" onclick="changeCategory(this);">測試自己控制力</button>
            <button id="imp_btn"  class="btn" onclick="changeCategory(this);">衝動、誘惑、或轉移注意</button>
            <button id="deal_btn"  class="btn" onclick="changeCategory(this);">與他人相處上遇到困難</button>
            <button id="soc_btn"  class="btn" onclick="changeCategory(this);">維持社交</button>
            <button id="party_btn"  class="btn" onclick="changeCategory(this);">與他人相聚</button>
        </div>
  </div>
...
//    drawRecentBarChart();
  ...
  //    setTimeout(function(){$(".clicklog_img").each(function(){$(this).attr('src', $(this).attr('_src'));})}, 1000);
   ...
   function drawThinkTable(){

   var patient_data = new google.visualization.DataTable();
   patient_data.addColumn('string', '日期&時間');
   patient_data.addColumn('string', '事件');
   patient_data.addColumn('string', '情緒');
   patient_data.addColumn('string', '想法');
   patient_data.addColumn('string', '預期情緒');
   patient_data.addColumn('string', '預期想法');
   patient_data.addColumn('string', '認同度');
   patient_data.addColumn('string', '是否反思正確');
   patient_data.addColumn('string', '備註');
  ...
  function changeCategory(caller){
  if(caller.className=="btn"){  
  caller.className = caller.className + " active";
  }
  else{
   caller.className="btn"
  }
